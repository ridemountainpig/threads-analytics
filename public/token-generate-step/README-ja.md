...
長期 Threads Access Token の有効期限は 60 日間ですが、通常は自分で再生成する必要はありません。

- データ同期のたびに Threads Analytics が token を確認し、残り 30 日を切ると自動的に更新します。更新に成功するたびに有効期限は再び 60 日間延長されます。
- Threads API は発行から 24 時間以上経過した token のみ更新できるため、貼り付けた直後の token は少し後に初回の自動更新が行われます。
- **設定**のアカウントカードに、token の有効期限と最後に自動更新された日時が表示されます。

> 自動更新が機能するのは、同期が継続して実行され、現在の token が有効な間だけです。アプリが長期間停止して token が期限切れになった場合は、上記の手順で新しい token を生成し、設定のアカウントカードの**トークンを更新**から貼り付けてください。
