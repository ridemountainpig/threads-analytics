...
| ツール                 | 機能                                                                                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `get_account_overview` | 接続済み全アカウントのユーザー名、同期状態、投稿数、データ期間、フォロワー成長サマリー——最初に呼ぶことを推奨                                        |
...
| `compare_periods`      | 2 期間のコア指標を絶対値・パーセント変化付きで比較。比較期間は直前の同じ長さの期間がデフォルト。AI キーを設定するとコンテンツカテゴリ別の内訳も表示 |
...
オプション。`AI_GATEWAY_API_KEY`（[Vercel AI Gateway](https://vercel.com/ai-gateway) のキー）を設定すると、高速な判定モデル Jev で投稿本文を読み取って分析する以下のツールも利用できます。大量の投稿を処理する場合もタイムアウトせずに `in_progress` を返し、もう一度呼び出すと続きから再開します。

| ツール                      | 内容                                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `sample_posts_for_taxonomy` | 好成績から低調まで均等に抽出した投稿サンプル。コンテンツカテゴリの草案づくりに使用（キー不要）                     |
| `evaluate_taxonomy`         | カテゴリ案をチェック：どれにも当てはまらない投稿、何でも入るカテゴリや重複するカテゴリ、成果の差を分けられているか |
| `classify_posts`            | すべての投稿を指定したカテゴリに分類し、カテゴリごとの成果を比較。当てはまらない投稿は `uncertain` として分離      |
| `score_draft`               | 下書きのフック、CTA、トピックを評価し、同じトピックの過去投稿と比較                                                |
| `compare_drafts`            | 2〜5 個の下書き案をランク付けし、それぞれがどの観点で優れているかを提示                                            |
| `analyze_post_features`     | どの書き方の特徴（体験談、具体的な数字、逆張りの主張、フックの強さなど）が閲覧数を伸ばすか                         |
| `analyze_thread_openers`    | スレッドの 1 件目から 2 件目へのつなぎ方が、2 件目の継続率にどう影響するか                                         |
...
| `build-taxonomy`       | 投稿に合うコンテンツカテゴリを見つけてチェックし、再利用できる設定として出力（AI キーが必要） |
...
常時稼働のデプロイ環境（Railway / Zeabur / VPS / Docker）では `SYNC_SCHEDULER_ENABLED=true` を設定します。内蔵スケジューラがサーバー起動とともに開始し、設定で構成された間隔で接続済みのすべてのアカウント（アクティブなものだけでなく）を同期します。Vercel では上記の cron 設定を使用してください。
...
| `AI_GATEWAY_API_KEY`     | AI 分析 MCP ツールを有効化（任意）           | Vercel AI Gateway で取得 |
