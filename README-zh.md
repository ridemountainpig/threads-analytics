...
| `compare_periods`      | 兩個期間的核心指標對照，附絕對與百分比變化；比較期間預設為主要期間正前方的等長視窗；設定 AI 金鑰後還能依內容分類拆解變化 |

連接多個帳號時，除了 `get_account_overview` 以外的每個工具都接受 `account` 參數（帳號名稱或 id）。若未提供，工具會回傳帳號清單，讓 agent 先問你要查哪一個，而不是自行猜測。

#### AI 分析工具

選用功能。設定 `AI_GATEWAY_API_KEY`（[Vercel AI Gateway](https://vercel.com/ai-gateway) 金鑰）後，伺服器會額外提供以下工具，用快速判斷模型 Jev 讀取貼文內文做分析。處理大量貼文時不會逾時，而是回傳 `in_progress`，再呼叫一次即可從進度接續。

| 工具                        | 功能                                                                                     |
| --------------------------- | ---------------------------------------------------------------------------------------- |
| `sample_posts_for_taxonomy` | 從表現最好到最差平均抽出貼文樣本，用來草擬內容分類（不需要金鑰）                         |
| `evaluate_taxonomy`         | 檢查草擬的分類：有沒有貼文哪類都不符合、分類是否變成雜物桶或彼此重疊、能否區分出成效差異 |
| `classify_posts`            | 把每篇貼文分到你定義的分類並比較各類成效；不太符合的貼文會另外歸為 `uncertain`           |
| `score_draft`               | 評估草稿的開頭吸引力、行動呼籲與主題，並與同主題的歷史貼文比較                           |
| `compare_drafts`            | 為 2–5 個草稿版本排名，並指出各版本贏在哪個面向                                          |
| `analyze_post_features`     | 哪些寫法特徵（個人故事、具體數字、反直覺觀點、開頭吸引力等）能提高觀看數                 |
| `analyze_thread_openers`    | 串文第一則如何銜接第二則，對第二則保留率的影響                                           |
...
| `build-taxonomy`       | 找出適合你貼文的內容分類，檢查後輸出可重複使用的分類設定（需要 AI 金鑰） |
...
| `AI_GATEWAY_API_KEY`     | 啟用 AI 分析 MCP 工具（選用）             | 從 Vercel AI Gateway 取得 |
